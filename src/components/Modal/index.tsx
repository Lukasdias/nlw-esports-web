...
export function Modal({ children, isOpen, toggle, onClose, type }: ModalProps) {
  const DefaultDialog = () => {
    return type === "add_ad" ? (
      <>
        <Dialog
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            <motion.div
              ...
              animate={"open"}
            >
              <Dialog.Panel className="w-[95%] mx-auto max-w-7xl  transform flex flex-col overflow-hidden rounded-2xl bg-background-secondary text-left align-middle shadow-xl transition-all px-10 py-8">
                {children}
              </Dialog.Panel>
            </motion.div>
          </div>
